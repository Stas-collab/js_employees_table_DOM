...
const pushNotification = (title, description, type)=>{
    ...
    if (type === "warning") container.classList.add("warning");
...
tBody.addEventListener("click", (e)=>{
    ...
    if (!tr) return;
    if (state.activeRow && state.activeRow !== tr) state.activeRow.classList.remove("active");
...
tHead.addEventListener("click", (e)=>{
    ...
    if (state.sortByIndex === columnIndex) state.sortDir = state.sortDir === "asc" ? "desc" : "asc";
    else {
        state.sortByIndex = columnIndex;
        state.sortDir = "asc";
    }
    if (state.sortDir === "asc") rows.sort((a, b)=>{
        ...
        if (isNaN(aText) && isNaN(bText)) return aText.localeCompare(bText);
        else return parseFloat(aText) - parseFloat(bText);
    });
    else rows.sort((a, b)=>{
        const aText = a.children[columnIndex].textContent.trim();
        const bText = b.children[columnIndex].textContent.trim();
        ...
        if (isNaN(aText) && isNaN(bText)) return bText.localeCompare(aText);
        else return parseFloat(bText) - parseFloat(aText);
    });
